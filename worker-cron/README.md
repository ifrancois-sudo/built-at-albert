...
Wakes on a schedule and calls the Pages Functions. It carries no business logic,
so the rules live in one place and the same endpoints can be tested by hand.
...
`APP_URL` lives in `wrangler.jsonc` and must point at the Pages site.
`CRON_SECRET` must be the same value on both sides.

## Test by hand

```bash
curl -X POST https://built-at-albert.pages.dev/api/cron/claims -H "Authorization: Bearer $CRON_SECRET"
```
